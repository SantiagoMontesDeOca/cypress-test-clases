...
export class TodoReactPage {
  ...
  // Check the checkbox based on its position.
  checkCheckboxByPosition(position) {
    cy.get(`:nth-child(${position}) > .view > .toggle`).should('be.checked');
  ...
  // Click on the button to filter completed tasks.
  clickFilterCompletedTasks() {
    cy.get('[data-reactid=".0.2.1.4"] > a').click();
  
  };

  //Click on the filter button corresponding to incomplete tasks.
  clickFilterIncompletedTasks() {
    cy.get('[data-reactid=".0.2.1.2"] > a').click();
  
  };
  
  // Click on the "All" button to display all tasks in the list again.
    clickFilterAllTasks() {
